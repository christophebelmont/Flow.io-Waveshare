...
coordonnées configurées avec `past_days=7`, `forecast_days=2` et
`timezone=auto`. La réponse est réduite à :

- température, couverture nuageuse et vent actuels ;
- sept journées calendaires passées avec températures minimale, maximale et
  moyenne, précipitations, nébulosité moyenne, vent maximal et rayonnement ;
- aujourd’hui et demain avec les mêmes agrégats prévisionnels.

Le parseur refuse les tableaux journaliers absents, désalignés, supérieurs à
neuf entrées ou ne contenant pas exactement sept jours passés. Le corps HTTP
est limité à 8 Kio et le document JSON à 12 Kio.
...
`PoolInsightPromptBuilder` produit trois chaînes bornées : les instructions
stables issues du prompt expert, un résumé météo lisible et les données
dynamiques. L’appel Responses transmet les deux dernières responsabilités dans
les champs distincts `instructions` et `input`. Le prompt interdit explicitement
les consignes de modification du pH, de l’ORP ou des automatismes déjà pilotés
par Flow.io.

La route locale `GET /api/ai/pool-preview` expose séparément les instructions,
les données dynamiques, l’état de l’analyse et son dernier texte. Le paramètre
`refresh=1` planifie auparavant un
...
aussi un buffer de 48 Kio en PSRAM ; aucune copie volumineuse n’est placée sur
...
La requête utilise `instructions` pour le comportement stable et `input` pour
les données Flow.io, limite la génération à 2048 jetons, raisonnement inclus,
et positionne `store` à `false`. Le parseur parcourt la collection `output` du
...
Le prompt inclut les sept journées closes, leur météo associée par date, les
statistiques jour/nuit, les activités de filtration et chauffage réparties en
quatre périodes locales, les consignes historiques, le remplissage, les
caractéristiques du bassin et la méthode de désinfection active. La journée en
cours reste fournie uniquement comme contexte complémentaire. La politique de
couverture est explicitement déclarative : aucune position réelle n’est
inventée en l’absence de capteur.

Les buffers temporaires OpenAI de requête HTTP (40 Kio), réponse HTTP (24 Kio)
et documents JSON (32 Kio chacun), ainsi que le document météo (12 Kio), les
données dynamiques (20 Kio), les instructions (8 Kio) et le résultat (4 Kio), utilisent tous la
