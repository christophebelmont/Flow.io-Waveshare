...
struct LocalUiBoardSpec {
    St7789DisplaySpec display;    // TFT hardware wiring and timings.
    LocalUiInputSpec inputs;      // Local UI input pins and behavior.
    LocalUpdateSpec update;       // Pins/settings for local update control.
};
...
struct BoardSpec {
    ...
    const LocalUiBoardSpec* localUi = nullptr; // Optional local display/input/update block.
